...
# original scan data — then coarse (step 0.2) from 1.0 up to 10.0.
GAMMA_LIST = ([round(0.1 * i, 10) for i in range(11)]             # 0.0 … 1.0  (11 vals)
              + [round(1.0 + 0.2 * i, 10) for i in range(1, 46)]) # 1.2 … 10.0 (45 vals)
J_DEFAULT  = 1.0
DT_DEFAULT = 0.05
N_H        = len(H_LIST)
N_G        = len(GAMMA_LIST)  # 56
N_TOTAL    = N_H * N_G   # 1120
