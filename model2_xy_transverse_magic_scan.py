"""Magic / framability scan of the anisotropic XY chain in a transverse field,
over a 2D parameter grid of anisotropy and field.

    H = J * sum_<ij> [ (1+gamma)/2 X_i X_j + (1-gamma)/2 Y_i Y_j ]
      + h * sum_i Z_i

with J = 1 and the scan axes

    gamma in linspace(-1, 1, 20)
    h     in linspace(0, 2, 20)

gamma = 0 is the gapless XX chain with a field-driven transition at h = J;
|gamma| = 1 recovers the transverse-field Ising model; (gamma, h) = (0, +-J)
is the multicritical point where the two critical lines meet.  See
unitary_models_for_magic.tex (Model 2).
...
Usage (local, one point):
    python model2_xy_transverse_magic_scan.py --self_test
    python model2_xy_transverse_magic_scan.py --p1 0.4 --p2 0.9
"""
...
# --- the anisotropic XY chain in a transverse field --------------------------
J_DEFAULT = 1.0
GAMMA_VALS = np.linspace(-1.0, 1.0, 20)
H_VALS = np.linspace(0.0, 2.0, 20)


def build_xy_transverse(gamma: float, h: float, J: float = J_DEFAULT):
    """(H1, H2) of  H = J sum_<ij> [(1+gamma)/2 XX + (1-gamma)/2 YY] + h sum_i Z."""
    H2 = J * (0.5 * (1.0 + gamma) * np.kron(_SX, _SX)
              + 0.5 * (1.0 - gamma) * np.kron(_SY, _SY))
    H1 = h * _SZ
    return H1, H2
...
def _model(J: float = J_DEFAULT) -> MagicModel:
    return MagicModel(
        name='xy_transverse',
        p1_name='gamma', p2_name='h',
        p1_vals=GAMMA_VALS, p2_vals=H_VALS,
        build=lambda g, h, _J=J: build_xy_transverse(g, h, _J),
        consts={'J': J},
    )


MODELS: dict[str, MagicModel] = {'xy_transverse': _model()}
...
# ---------------------------------------------------------------------------
def self_test() -> None:
    model = MODELS['xy_transverse']

    # 1. Grid.
    assert model.shape == (20, 20), model.shape
    assert abs(model.p1_vals[0] + 1.0) < 1e-12 and abs(model.p1_vals[-1] - 1.0) < 1e-12
    ...
    H1, H2 = model.build(0.4, 0.9)
    assert np.allclose(H2, H2.conj().T) and np.allclose(H1, H1.conj().T)
    # ||H||_1 = |J|(1+gamma)/2 (XX) + |J|(1-gamma)/2 (YY) + |h|(Z)
    from trotter_lindbladian_scan import DT_BASE
    expect = DT_BASE / (0.5 * 1.4 + 0.5 * 0.6 + 0.9)
    assert abs(point_dt(model, 0.4, 0.9) - expect) < 1e-15, point_dt(model, 0.4, 0.9)
    dt_min = min_dt_over_grid(model)
    # (1+gamma)/2 + (1-gamma)/2 == 1 identically on gamma in [-1, 1]; the norm
    # is minimized (dt maximized... i.e. dt_min hit) at h = 2 (largest field).
    assert abs(dt_min - DT_BASE / (1.0 + 2.0)) < 1e-15, dt_min

    # 3. dt ladder.
    dts = dt_ladder(point_dt(model, 0.4, 0.9))
    ...
    #    over unchanged).
    H = ring_hamiltonian(model, 0.4, 0.9, 4)
    assert H.shape == (16, 16) and np.allclose(H, H.conj().T)
    assert len(bonds_ring(4)) == 4
    # trace = 0: X, Y, Z, XX, YY are all traceless.
    assert abs(np.trace(H)) < 1e-10
    # h -> -h is a spin flip (X^{(x)4} conjugation) of the gamma-dependent part.
    flip = _site_op(_SX, 0, 4) @ _site_op(_SX, 1, 4) @ _site_op(_SX, 2, 4) @ _site_op(_SX, 3, 4)
    assert np.allclose(flip @ H @ flip, ring_hamiltonian(model, 0.4, -0.9, 4), atol=1e-10)
    # Other ring sizes: right shape, Hermitian.
    for n in (5, 6):
        Hn = ring_hamiltonian(model, 0.4, 0.9, n)
    ...
    print('model2_xy_transverse_magic_scan self-test passed.')
...
def main() -> None:
    ...
    ap.add_argument('--model', default='xy_transverse', choices=list(MODELS))
    ap.add_argument('--p1', type=float, help='first parameter (gamma)')
