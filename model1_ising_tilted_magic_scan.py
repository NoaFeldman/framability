...
from dissipative_PT import _SX, _SZ, _site_op
...
def self_test() -> None:
    ...
    #    over unchanged).
    H = ring_hamiltonian(model, 0.7, 0.3, 4)
    ...
    assert np.allclose(flip @ H @ flip, ring_hamiltonian(model, 0.7, -0.3, 4), atol=1e-10)
    # Other ring sizes: right shape, Hermitian.
    for n in (5, 6):
        Hn = ring_hamiltonian(model, 0.7, 0.3, n)
