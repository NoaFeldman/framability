...
#
#  Usage:
#    bash submit_product_schro.sh [--n_pts N] [--J J] [--gamma_step S]
...
echo "========================================================"
echo "  Product-state Schrödinger framability  (chi=30)"
echo "  N_PTS        = ${N_PTS}"
echo "  J            = ${J}"
echo "  GAMMA_STEP   = ${GAMMA_STEP}"
echo "  OUT_DIR      = ${OUT_DIR}"
echo "  Max jobs     = ${MAX_CONCURRENT} concurrent"
echo "  Array tasks  = 0-${ARRAY_END}  (${N_PTS}x${N_PTS} = $((N_PTS * N_PTS)))"
if [[ -n "$AFTER_JOB" ]]; then
...
COLLECT_JOB_ID=$(
    ...
    sbatch --parsable \
           ...
           --job-name=prod_schro_collect \
           --ntasks=1 --cpus-per-task=1 --mem=8G --time=00:15:00 \
           --output="logs/prod_schro_collect_%j.out" \
           --error="logs/prod_schro_collect_%j.err" \
           --wrap="source '${SLURM_SUBMIT_DIR:-$(pwd)}/.venv/bin/activate' && \
                   export MPLCONFIGDIR=/tmp/matplotlib-\${SLURM_JOB_ID} && \
                   python scripts/product_schro_collect.py \
                       --n_pts '${N_PTS}' --J '${J}' \
                       --gamma_step '${GAMMA_STEP}' --out_dir '${OUT_DIR}'"
)
