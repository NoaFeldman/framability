...
#  Heisenberg framability RATES (rate_heis_4 / rate_heis_6) over the full grid
#  of one rate-panel model (MODEL, default model4).
#
...
#    MODEL=model10 ROUND=1 sbatch --job-name=m10_qrefine scripts/model4_rate_quick_refine.slurm.sh
...
#
#  Output: results_<model>_rate/<model>/pt_<ix>_<iy>_qrefine_r<NN>.npz
...
echo "[${MODEL} rate qrefine] round ${ROUND}, chunk ${SLURM_ARRAY_TASK_ID}/${N_CHUNKS}: starting"
...
echo "[${MODEL} rate qrefine] round ${ROUND}, chunk ${SLURM_ARRAY_TASK_ID}: done"
