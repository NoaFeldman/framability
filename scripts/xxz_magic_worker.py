...
Usage (strided across a 200-task array):
    python scripts/xxz_magic_worker.py --task_id $SLURM_ARRAY_TASK_ID --n_chunks 200
"""
...
def _is_current(out: Path, groups: tuple[str, ...]) -> bool:
...
def _merge_previous(out: Path, save: dict) -> dict:
...
def run_point(model, point_id: int, dt_min: float, args) -> None:
    ...
    if not args.force and _is_current(out, groups):
        print(f'[skip] {model.name}/{out.name} already at version '
              f'{XXZ_MAGIC_VERSION}', flush=True)
        return
...
def main() -> None:
    ...
    p.add_argument('--model',    type=str, default='xxz', choices=list(MODELS))
    ...
    print(f'[{model.name}] grid {model.shape[0]}x{model.shape[1]} = {N} points, '
          f'dt_min = {dt_min:.6g}', flush=True)

    if args.n_chunks <= 1:
        if not (0 <= args.task_id < N):
            print(f'ERROR: task_id must be in [0, {N})', file=sys.stderr)
            sys.exit(1)
        run_point(model, args.task_id, dt_min, args)
        return
    ...
    for pid in point_ids:
        ...
        try:
            run_point(model, pid, dt_min, args)
        except Exception:
            n_fail += 1
            import traceback
            traceback.print_exc()
