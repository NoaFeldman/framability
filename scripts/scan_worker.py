...
from optimize_framability import minimize_framability, DEFAULT_METHOD, OPT_VERSION
...
def main():
    ...
    # Version sidecar: stamps the framability-optimiser version this row was
    # produced with, so build_two_qubit_scan_full can detect and re-run rows
    # generated by an older code version (.npy carries no metadata of its own).
    with open(os.path.join(args.out_dir, f'row_{ig:04d}.ver'), 'w') as fh:
        fh.write(OPT_VERSION)
    print(f'[task {ig}] wrote {out_path}  (version {OPT_VERSION})', flush=True)
